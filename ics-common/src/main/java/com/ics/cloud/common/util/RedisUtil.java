...
public class RedisUtil {
     ...
     * 加锁
     *
     * @param key   唯一标志（如果锁商品就是商品id，如果锁用户就是用户id）
     * @param value 当前时间+超时时间 也就是时间戳
     * @return
     */
    public boolean lock(String key, String value) {
        return lock(key, value, 120 * 1000);
    }
     ...
     * @param key      唯一标志（如果锁商品就是商品id，如果锁用户就是用户id）
     * @param value    当前时间+超时时间 也就是时间戳
     * @param waitTime 阻塞等待时间 毫秒
     * @return
     */
    public boolean lock(String key, String value, int waitTime) {
        ...
        while (System.currentTimeMillis() - start < waitTime) {
            try {
                ...
                if (redisTemplate.opsForValue().setIfAbsent(key, value)) {//对应setnx命令
                    //可以成功设置,也就是key不存在
                    //redisTemplate.expire(key,expreTime, TimeUnit.MILLISECONDS);
                    return true;
                }
                ...
                log.debug(redisTemplate.getExpire(key) + "");
                log.debug("lock wait key={},value={}", key, value);
     ...
     * @return
     */
    public boolean sendSubscribeMsg(String changel, String msg) {
        redisTemplate.convertAndSend(changel, msg);
        return true;
    }
}
